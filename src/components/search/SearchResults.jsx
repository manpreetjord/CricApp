...
function SearchResults({ matches, players, isVisible, onClose }) {
  ...
  return (
    <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 rounded-md shadow-lg overflow-hidden z-30 max-h-96 overflow-y-auto border border-gray-200 dark:border-gray-700">
      <div className="p-2">
        {matches.length > 0 && (
          <div className="mb-4">
              ...
              {matches.map((match) => (
                <li key={match.id}>
                  <Link
                    ...
                    className="block px-3 py-2 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-md transition text-gray-800 dark:text-gray-200"
                    onClick={onClose}
                  >
                    ...
                    <div className="flex justify-between mt-1 text-sm">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-xs text-white font-medium ${
                        match.status === 'Live' ? 'bg-red-600 dark:bg-red-500' : 'bg-blue-600 dark:bg-blue-500'
                      }`}>
                        {match.status}
                      </span>
                    </div>
              ...
              {players.map((player) => (
                <li key={player.id || player.name}>
                  <Link
                    ...
                    className="block px-3 py-2 hover:bg-green-50 dark:hover:bg-green-900/30 rounded-md transition text-gray-800 dark:text-gray-200"
                    onClick={onClose}
                  >
                    <div className="flex justify-between">
        ...
        {matches.length === 0 && players.length === 0 && (
          <div className="px-3 py-4 text-center text-gray-600 dark:text-gray-400">
            No results found
          </div>
        )}
      </div>
