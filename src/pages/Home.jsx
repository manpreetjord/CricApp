...
function Home() {
  ...
  useEffect(() => {
    // Get all available matches for quick links
    const matches = fetchLiveMatches()
    setAvailableMatches(matches)
  }, [])
  
  // Get first match ID for quick links
  const firstMatchId = availableMatches.length > 0 ? availableMatches[0].id : 1
    ...
    <div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Link to={`/live-match/${firstMatchId}`} className="bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-md py-3 px-4 text-center font-medium hover:from-blue-600 hover:to-blue-700 transition duration-200">
            Live Match
          </Link>
          ...
          <Link to={`/match/${firstMatchId}`} className="bg-gradient-to-r from-yellow-500 to-yellow-600 text-white rounded-md py-3 px-4 text-center font-medium hover:from-yellow-600 hover:to-yellow-700 transition duration-200">
            Match Details
          </Link>
        </div>
      </div>
